...
def ot_mi(X,bins):
   ...
   # compute ot distance/ emd distance
    ...
    ot_sim = affinityMatrix(ot_dist,K = 20)
    ot_sim = ot_sim / torch.max(ot_sim)
...
def ot_mi2(X):
   ...
   # compute ot distance/ emd distance
    ot_dist = np.zeros((n, n))
    for i in np.arange(n):
        for j in np.arange(i+1, n):
            ot_dist[i,j] = ot.emd2_1d(X[:,i], X[:,j])
...
def compute_obj(X1,X2,S,W1,W2,H1,H2,D1,A1,D2,A2,alpha,beta,gamma,phi):
    ...
    obj = obj + alpha *( torch.square(torch.norm(S - H1@H1.T )) + torch.square(torch.norm(S-H2@H2.T)) )
    obj += gamma * (torch.trace(H1.T @ L1 @ H1 + H2.T @ L2 @ H2 ))
    allones_vec = torch.ones(n,1).cuda()
    allones_mat = torch.ones(n,n).cuda()
    obj += beta * torch.trace((H1@H1.T) @ (allones_mat - H2@H2.T)) + phi * torch.square(torch.norm(S@allones_vec - allones_vec))
